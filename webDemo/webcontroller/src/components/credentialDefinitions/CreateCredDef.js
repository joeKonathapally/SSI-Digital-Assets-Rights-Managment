import {createCredDef} from "../../helper/cred_def/createCredDef";
...
function CreateCredDef(props){
  ...
  async function createCredDefSubmit(event){
    ...
    let txn = await createCredDef(props.api_url, connId, endorseTransaction, bodyRaw);
  ...
  return(
    <>
      <div>
        <form onSubmit={(event) => createCredDefSubmit(event)} id="filtery">
        <label>
            Body:
            <br/>
          ...
          <br/>
          <button type="submit">Create credential definition</button>
        </form>
      </div>
    </>
  );
